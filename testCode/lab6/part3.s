...
KEY_ISR:    
    LDR      R3, =0xFF200050 // Load KEY addr
    LDR      R2, [R3, #0xC]  // Load KEY edge
    ...
    LDR      R3, =0xFF202000 // Load timer
    MOV      R0, #8
    STR      R0, [R3, #0x4]  // Stop the timer

    LDR      R0, [R3, #8]       // Get lower bits of timer
    LDR      R1, [R3, #0xC]     // Get upper bits of timer
    LSL      R1, #16
    ORR      R1, R0, R1
...
KEY1_:
    LSR     R1, #1      // Half timer decrement time
    STR     R1, [R3, #8]
...
KEY2_:
    LSL     R1, #1      // Double timer decrement time
    STR     R1, [R3, #8]
